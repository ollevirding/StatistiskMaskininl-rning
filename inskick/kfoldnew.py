...
def kfold(x,y,k,model, norm = False, **args):
    ...
    for i,ind in enumerate(splitInd):
            if i == len(splitInd)-1:
                ...
                train = traincompleteshuffle.iloc[:ind]

            else:
                r=[i for i in range(ind, splitInd[i+1])]  
                rnot = [i for i in range(ind)] + [i for i in range(splitInd[i+1], l)]            
                hold = traincompleteshuffle.iloc[r]
            ...
            ball = met.balanced_accuracy_score(prediction, holdy)
            conf.append(confmat)
            Ehold.append(acc)
            balacc.append(ball)
    balaccavg = np.average(balacc)
    ...
    return goodm, Eholdavg, confarr, balaccavg
